...
import { isLayoutGroup, type UINode } from "@pixi-ui-editor/schema";
...
export function TransformSection({ node, readOnly }: { node: UINode; readOnly: boolean }) {
  ...
  const managedByLayout = parent !== undefined && isLayoutGroup(parent);
  ...
  return <>
    ...
    <InspectorWindow title="Transform">
      {managedByLayout && <p className="inspector-hint">Position, anchors and size are controlled by the parent Layout Group.</p>}
      {managedByLayout && <InspectorWindow title="Layout Item">
